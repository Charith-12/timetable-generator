...
int COST_OF_NEW_LECTURER = 350000; // Per month
int COST_TO_INCREASE_WL = 10000; // Per hour (credit)
int COST_TO_EXTEND_HOURS = 0; // Extend by 1 hour considering 1 week
int COST_OF_NEW_CLASSROOM = 500000; // 1 new classroom
// * least pay_rate < COST_TO_INCREASE_WL (per hour) << COST_OF_NEW_LECTURER

int totalCost = 0;
int costIncr = 10; // cost of adding one extra timeslot
int newRoomCost = 100; //cost of adding one extra classroom.
int NO_OF_NEW_LECTURERS = 0;
int NO_OF_NEW_CLASSROOM = 0;
...
struct ua_lecturer { // un-allocated lecturer
  char lecid[8];
  int payRate; 
  int availHours; // initial value: max_hours of work load (work load limit). decreases as modules getting assigned.
} UALecturers[MAX_LECTURERS];;

struct lecForMod {
	char lecid[8];
  	struct lecForMod* link;
};

struct lecModPair {
	char lecid[8];
	char modid[8];
  	struct lecModPair* link;
};
...
void setCostAmounts(); // function prototype for getting lec, wl, extention, and classroom costs from the DB and set.

int getUALecturers(); // function prototype for getting all un-allocated lecturers, returns no. of lecturers. 
struct lecForMod* getLecturersForModule(char mod_id[]); // A function prototype to get all lecturers who teach a certain module.
struct lecForMod* addNewLecForModToLL(char lecid2[], struct lecForMod* h); // A function prototype to add new LecForMod node to the linked list of lecturers for a particular module. 
void clearLecForModLL(struct lecForMod** head_ref); // A function prototype to delete a linked list of lecForMod structs.
int allocateBestLecturerToModule(int num_of_lecturers); // A function prototype to allocate best lecturer for each module.
struct lecModPair* addNewAssignedLecModPairToLL(char lecid2[], char modid2[], struct lecModPair* h); // A function prototype to add new assigned lecModPair to the linked list of finalized lecturer-module pairs.
int putAssignedLecModPairLL(struct lecModPair* assignedLLHead, char tablename[]); // A function prototype to put assigned lec-mod pairs Linked List into the Database.
void clearAssignedLecModPairLL(struct lecModPair** head_ref); // A function prototype to delete a linked list of lecModPair structs.
...
int main(void) {
  ...
  // get cost amounts from DB.
  setCostAmounts();
  
  // New 24.09 - performing best lmallocation manually
  // have payrate and wl limit to be sent to the lecturers table in C db from frontend. Have new 'lecmodallocation' table which stores multiple lecturers for a 1 module. Maybe remove unique modules options in the front end.
  num_lecturers = getUALecturers(); // populating UALecturers array in wl descending order. also getting the lecturers count.
  if(allocateBestLecturerToModule(num_lecturers))  printf("\n Optimum lecturer was assigned to each of the modules successfully! \n");
...
//  displayClassroomsLL(copyOfclassroomsLL_head);
  ...
  printf("\n Number of new lecturers needed to hire = %d \n", NO_OF_NEW_LECTURERS);
...
// get lec, wl, extention, and classroom costs from the DB and set.
void setCostAmounts(){
	...
	/* send SQL query */
	if (mysql_query(conn, "SELECT new_lec, wl_ext, time_ext, new_room FROM costs")){
	 	printf("Failed to execute query to get cost amounts. Error: %s\n", mysql_error(conn));
	  	mysql_close(conn);
	  	return;
	}
	...
	while(row = mysql_fetch_row(res)){
		COST_OF_NEW_LECTURER = row[0] ? atoi(row[0]) : 350000;
		COST_TO_INCREASE_WL = row[1] ? atoi(row[1]) : 10000;
		COST_TO_EXTEND_HOURS = row[2] ? atoi(row[2]) : 0;
		COST_OF_NEW_CLASSROOM = row[3] ? atoi(row[3]) : 500000;

 	}
 	printf("\n + Cost amounts retrived from the DB\n");
...
// A function to get un-allocated lecturer details from the DB, returns the total number of lecturers.
int getUALecturers() {
	...
	while(row = mysql_fetch_row(res)){
		...
		strcpy(UALecturers[i].lecid, (row[0] ? row[0] : "NULL")); //printf("%s ", row[0] ? row[0] : "NULL");	
  		UALecturers[i].payRate = (row[1] ? atoi(row[1]) : 0); //printf("%d ", row[1] ? atoi(row[1]) : 0);
  		UALecturers[i].availHours = (row[2] ? atoi(row[2]) : 168);	//printf("%d ", row[2] ? atoi(row[2]) : 0); //168 = 24hrs x 7days
		// atoi: int converting
		i++;
 	}
...
// A function to get all lecturers who teach a certain module
struct lecForMod* getLecturersForModule(char mod_id[]){
	...
	/* send SQL query */
	if (mysql_query(conne, queryStr)){
	 	printf("Failed to execute query to get lecturerIDs for allocation. Error: %s\n", mysql_error(conne));
	  	mysql_close(conne);
	  	return NULL;
	}
	
	res = mysql_store_result(conne);
	if (res == NULL){
		mysql_close(conne);
		return NULL;
    }
	...
	while(row = mysql_fetch_row(res)){
		...
		head = addNewLecForModToLL( (row[0] ? row[0] : "NULL"), head);
...
// A function to add new LecForMod node to the linked list of lecturers for a particular module. 
struct lecForMod* addNewLecForModToLL(char lecid2[], struct lecForMod* h){
	
	struct lecForMod* loop=NULL;
	struct lecForMod* prev=NULL;
	
 	//create new pointer to point to a "new block" of memory 
    struct lecForMod* new=malloc(sizeof(struct lecForMod));
...
// A function to delete a linked list of lecForMod structs 
void clearLecForModLL(struct lecForMod** head_ref){
   /* deref head_ref to get the real head */
   struct lecForMod* current = *head_ref;
   struct lecForMod* next;
...
// A function to allocate best lecturer for each module
int allocateBestLecturerToModule(int num_of_lecturers){
	
	// 2. for each module in new 'lecmodallocation' table, get credit count & all available lec_ids in payrate ascending order. store those in a temp LL?
    // 3. for each lec_id from the start, go through the ua_lecturers LL and find if the avail_wl >= modules's credit amount.
    // 4. if no, move onto next lec_id in temp LL
    // 5. if yes, reduce the avail_wl of the lecturer matching to the credit count. 
    // 6. add mod_id and that particular lec_id to the LL of lmalloc struct.
    // 7. calculate and increment the lecturer cost.
    // 8. after all modules in 'lecmodallocation' are considered, send populated LL of lmalloc struct to lmallocations table in C db.
	...
	/* send SQL query */
	if (mysql_query(conn, queryStr)){
	 	printf("Failed to execute query to get modules for allocation. Error: %s\n", mysql_error(conn));
	  	mysql_close(conn);
	  	return 0;
	}
	...
	while(row = mysql_fetch_row(res)){  // module
		
		//strcpy(moduleId, (row[0] ? row[0] : "NULL"));
		moduleCredits = row[1] ? atoi(row[1]) : 0;
		
		head = getLecturersForModule( (row[0] ? row[0] : "NULL") ); // param: mod_id
		
		struct lecForMod* loop = NULL;
		loop = head;
		
		while(loop!=NULL){  // 1 lecturer in teachable list. list is sorted in payRate ascending order
			
			// Choosing the best lecturer
			isLecturerAssigned=0; i=0;
			for(i=0; i<num_of_lecturers; i++){
				if(strcmp(UALecturers[i].lecid, (*loop).lecid) == 0){
					if( (UALecturers[i].availHours >= moduleCredits) /*&& (UALecturers[i].payRate <= COST_TO_INCREASE_WL)*/ ){  // if instead of allocating next best lecturer, using the the least cost lecturer by extending WL is cheaper.
						
						//add to final assigned lec-mod pairs linked list.
						assignedLLHead = addNewAssignedLecModPairToLL((*loop).lecid, (row[0] ? row[0] : "NULL"), assignedLLHead);
						UALecturers[i].availHours = (UALecturers[i].availHours)-moduleCredits;
						totalLecturerCost = totalLecturerCost + (UALecturers[i].payRate * moduleCredits);
						isLecturerAssigned=1;
					} else{
						isLecturerAssigned=0;
					}
					break;
				}
			}
			if(isLecturerAssigned==1){
				break;
			}
    	...
    	if(isLecturerAssigned==0){ // no lecturer is available to assign in the list. 
    		if( COST_OF_NEW_LECTURER < (COST_TO_INCREASE_WL*moduleCredits*4) ){ // hiring new lecturer is cheaper
    			// hire a new lecturer. assigning 'NH~1'...
    			NO_OF_NEW_LECTURERS++;
    			char new_lecid[8];
    			sprintf(new_lecid, "NH~%d", NO_OF_NEW_LECTURERS); // New Hire
    			
    			assignedLLHead = addNewAssignedLecModPairToLL(new_lecid, (row[0] ? row[0] : "NULL"), assignedLLHead);
    			
			} else { // increasing the work load of the current cheapest existing lecturer is the cheaper path
				assignedLLHead = addNewAssignedLecModPairToLL((*head).lecid, (row[0] ? row[0] : "NULL"), assignedLLHead);
			}
	...
	// PUSH to lmallocations table.
 	if(putAssignedLecModPairLL(assignedLLHead, "lmallocations")) printf("\n Optimum Lecturer-Module allocations added to the database successfully! \n");
	
	clearAssignedLecModPairLL(&assignedLLHead);
	free(assignedLLHead); assignedLLHead=NULL;

 	return 1;
}

// A function to add new assigned lecModPair to the linked list of finalized lecturer-module pairs.
struct lecModPair* addNewAssignedLecModPairToLL(char lecid2[], char modid2[], struct lecModPair* h){
	
	struct lecModPair* loop=NULL;
	struct lecModPair* prev=NULL;
	
 	//create new pointer to point to a "new block" of memory
    struct lecModPair* new=malloc(sizeof(struct lecModPair));
  
    //set data and the link
  	strcpy((*new).lecid, lecid2);  // lecturer id
  	strcpy((*new).modid, modid2);  // module id
...
// A function to put assigned lec-mod pairs Linked List into the Database
int putAssignedLecModPairLL(struct lecModPair* assignedLLHead, char tablename[]){
	...
	//
	// ADD DROP TABLE IF EXISTS THEN CREATE lmallocations TABLE
	sprintf(queryStr, "DROP TABLE IF EXISTS `%s`", tablename);	
	if (mysql_query(conn, queryStr)){
    	printf("\n Failed to drop lmallocations table! \nError: %s\n", mysql_error(conn));
    	mysql_close(conn);
    	return 0;
  	}
	sprintf(queryStr, "CREATE TABLE `%s` (`lec_id` varchar(255) NOT NULL, `mod_id` varchar(255) NOT NULL)", tablename);	
	if (mysql_query(conn, queryStr)){
    	printf("\n Failed to create lmallocations table! \nError: %s\n", mysql_error(conn));
    	mysql_close(conn);
    	return 0;
  	}
  	sprintf(queryStr, "ALTER TABLE `%s` ADD PRIMARY KEY (`mod_id`)", tablename);
  	if (mysql_query(conn, queryStr)){
    	printf("\n Failed to set Primary key in lmallocations table! \nError: %s\n", mysql_error(conn));
    	mysql_close(conn);
    	return 0;
  	}
	//
	...
	loop = assignedLLHead;
	while(loop!=NULL){
		sprintf(queryStr, "INSERT INTO %s (lec_id, mod_id) VALUES('%s', '%s')", tablename, (*loop).lecid, (*loop).modid);
		
		/* send SQL query */	
		if (mysql_query(conn, queryStr)){
			printf("\n Query Error! Failed to insert lmallocations to the DB. Error: %s\n", mysql_error(conn));
	    	mysql_close(conn);
	    	return 0; 
		}
...
// A function to delete a linked list of lecModPair structs 
void clearAssignedLecModPairLL(struct lecModPair** head_ref){
   /* deref head_ref to get the real head */
   struct lecModPair* current = *head_ref;
   struct lecModPair* next;
...
struct subject* getSubjects(char lecid[], int nos){
    ...
    printf("\n getting subjects of the lecturer %s", lecid);
...
// A function to add new classroom to the linked list of classrooms.
