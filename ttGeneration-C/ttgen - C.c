...
struct node* insert(int d, int s, struct node* h); // function prototpe for adding nodes to the linked list of nodes (tt)
...
int main(){
  ...
  if (maxTtTimeSlots < totalSlotReq) printf("\n Error!!! Total timeslot requirement exceeds the possible no. of timeslots.\n\n"); // TODO: Fix this. maybe handle this while allocating. 
  	...
  	int V = maxTtTimeSlots;  // TODO: we need more slots than the rquirement(and maybe even more than the max possible no.) when allocating breaks inbetween.
