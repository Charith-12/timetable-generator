...
struct batch* insertModules(char modid2[], int noc2, char lecid2[], struct module* h); //function prototpe for inserting a new node to the modules linked list
...
void paintDigit(struct batch* n); //to paint the digit on the screen
void unwrap(int wrap, int flag); //print the wrapped number of symbols. wrap=number to repeat and flag=0/ 1
//void paintContour(struct number* n);
...
//  printf("\n Total timeslots required for batchOne : %d \n\n", slotCount);
  ...
  int totalSlotReq = batchOne.slotsTotal; // just substituting for now 
    ...
    // TODO: IMPORTANT! we need more than 'slotsTotal' when allocating breaks inbetween.
...
//		  	printGraph(graph);
  	...
  	printf("\n\n - Populating the timetable using modules linked list for the batchOne --->\n\n");
  		// count for batch's total slots - k
	  	// count for modules's total slots - k
	  	// count for lecturer's workload. get lecid out of the module struct and store globally.
	  	// check if slot's occupied - k
	  	// allocate breaks, check if previous slot was a break/lecture. - k
...
struct batch* insertModules(char modid2[], int noc2, char lecid2[], struct module* h){	
...
void paintDigit(struct batch* n){
  int i=0; //count data elements
  int f=1; //the font to print
  int d=-2; //to store pattern data element to print/ display
  
  //create a pointer variable to loop through the linked list
	struct module* loop=NULL;
	
	loop=(*n).modules; //refer to npattern member of n
	while(loop!=NULL){
	  //assign data to display
	  d=(*loop).noc; //printf("[%d]", d);
	  
	  if(d==-1){ //if end of row
	    i=0;
	    printf("\n"); //drop a line	    
	  }else{
	    ++i; //printf("%d", i);
	    if(i%2==1){ //if odd position=white/ zero
	      f=32; //print with ASCII 32=space character
	      unwrap(d, f); //paint
	    }else{
	      f=46; //print with ASCII 1
	      unwrap(d, f); //paint
	    }
	  }
  	//printf("| %d |", (*loop).data); printf("-->"); //printf(" %p |-->", (*loop).link);
  	loop=(*loop).link; //go to next data element
  }
  printf("\n");
}

void unwrap(int wrap, int flag){
  int i=0;
  for(i=0; i<wrap; i++){
    printf("%c", flag);
  }
